...
class TestTranscriptionConfig:
    ...
    def test_default_config(self):
        ...
        assert config.model_name == "nezamisafa/whisper-persian-v4"
    ...
    def test_config_validation(self):
        ...
        with pytest.raises(
            ValueError, match="chunk_duration_ms must be positive"
        ):
        ...
        with pytest.raises(
            ValueError, match="overlap_ms must be less than chunk_duration_ms"
        ):
            TranscriptionConfig(chunk_duration_ms=1000, overlap_ms=1000)

        # Test invalid repetition threshold
        with pytest.raises(
            ValueError, match="repetition_threshold must be between 0 and 1"
        ):
            TranscriptionConfig(repetition_threshold=1.5)

        # Test invalid max word repetition
        with pytest.raises(
            ValueError, match="max_word_repetition must be at least 1"
        ):
    ...
    def test_config_clone(self):
        ...
        original = TranscriptionConfig(
            model_name="base", language="en", chunk_duration_ms=15000
        )
