...
class SentenceExtractor:
    ...
    def format_sentence_preview(sentences: List[str], part_number: int) -> str:
        ...
        try:
            ...
            if sentences:
                terminal_display.print_persian_preview(
                    sentences[0], part_number
                )
                ...
                if len(sentences) > 1:
                    for i, sentence in enumerate(sentences[1:], 2):
                        if sentence.strip():
                            terminal_display.print_simple_preview(
                                sentence, f"{part_number}.{i}"
                            )

            return (
                ""  # Return empty string since we're handling display directly
            )
        except ImportError:
            ...
            for i, sentence in enumerate(sentences, 1):
                ...
                if i == 1:
                    preview_lines.append(
                        f"Part {part_number} Preview: {display_sentence}"
                    )
                else:
    ...
    def split_into_paragraphs(
        text: str, min_paragraph_length: int = 50
    ) -> List[str]:
