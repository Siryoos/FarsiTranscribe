...
class AdvancedModelEnsemble:
    ...
    def _setup_models(self):
        ...
        self._load_model(
            "wav2vec2", "m3hrdadfi/wav2vec2-large-xlsr-persian-v3", 0.3
        )
    ...
    def transcribe_ensemble(
        ...
        for name, model in self.models.items():
            try:
                if "whisper" in name:
                    text, confidence = self._transcribe_whisper(
                        name, audio_data
                    )
                elif "wav2vec2" in name:
                    text, confidence = self._transcribe_wav2vec2(
                        name, audio_data
                    )
                else:
                    text, confidence = self._transcribe_generic(
                        name, audio_data
                    )
        ...
        final_text, ensemble_confidence = self._ensemble_results(
            results, confidences
        )
    ...
    def _transcribe_whisper(
        self, name: str, audio_data: np.ndarray
    ) -> Tuple[str, float]:
        ...
        confidence = (
            0.8  # Placeholder - would need logprobs for accurate confidence
        )
    ...
    def _ensemble_results(
        self, results: Dict[str, str], confidences: Dict[str, float]
    ) -> Tuple[str, float]:
        ...
        # Select best result
        best_model = max(
            weighted_scores.keys(), key=lambda k: weighted_scores[k]
        )
