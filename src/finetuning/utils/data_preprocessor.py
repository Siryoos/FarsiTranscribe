...
class AudioDataPreprocessor:
    ...
    def preprocess_audio_file(self, audio_path: str, output_path: Optional[str] = None) -> str:
        """
        ...
        try:
            ...
            self._save_audio(processed_audio, output_path, self.config.target_sample_rate)
        ...
        for audio_file in input_path.rglob('*'):
            if audio_file.suffix.lower() in audio_extensions:
                try:
                    ...
                    output_file = output_path / relative_path.with_suffix(f'.{self.config.target_format}')
                    output_file.parent.mkdir(parents=True, exist_ok=True)
                    
                    processed_path = self.preprocess_audio_file(str(audio_file), str(output_file))
    ...
    def _apply_preprocessing_pipeline(self, audio: np.ndarray, sr: int) -> np.ndarray:
        ...
        if duration < self.config.min_duration or duration > self.config.max_duration:
            raise ValueError(f"Audio duration {duration:.2f}s outside allowed range [{self.config.min_duration}, {self.config.max_duration}]")
        
        # Amplitude filtering
        if np.max(np.abs(audio)) < self.config.min_amplitude:
            raise ValueError(f"Audio amplitude too low: {np.max(np.abs(audio)):.4f}")
    ...
    def _reduce_noise(self, audio: np.ndarray) -> np.ndarray:
        ...
        gate_threshold = noise_estimate * (1 + self.config.noise_reduction_strength)
        magnitude_gated = np.where(magnitude > gate_threshold, magnitude, gate_threshold)
    ...
    def _generate_output_path(self, input_path: str) -> str:
        ...
        return str(output_dir / f"{input_path_obj.stem}_processed.{self.config.target_format}")
    ...
    def create_preprocessing_report(self, input_files: List[str], output_files: List[str]) -> Dict:
        ...
        for file_path in input_files:
            ...
            report["input_formats"][ext] = report["input_formats"].get(ext, 0) + 1
        
        # Analyze output formats
        for file_path in output_files:
            ext = Path(file_path).suffix.lower()
            report["output_formats"][ext] = report["output_formats"].get(ext, 0) + 1
