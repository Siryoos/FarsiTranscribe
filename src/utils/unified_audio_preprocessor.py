...
class PersianFrequencyOptimizer:
    ...
    @staticmethod
    def create_persian_emphasis_filter(
        sample_rate: int,
    ...
    @staticmethod
    def enhance_persian_consonants(
        audio_samples: np.ndarray, sample_rate: int
    ) -> np.ndarray:
        """Enhance Persian consonant clarity."""
        # Emphasis for Persian fricatives (ش، ژ، خ، غ)
        high_freq_b, high_freq_a = scipy.signal.butter(
            4, [3000, 7000], btype="band", fs=sample_rate
        )
        consonant_enhanced = scipy.signal.filtfilt(
            high_freq_b, high_freq_a, audio_samples
        )
    ...
    def __init__(self):
        ...
        if FACEBOOK_DENOISER_AVAILABLE:
            try:
                ...
                self.logger.warning(
                    f"Facebook Denoiser initialization failed: {e}"
                )
                self.model = None
        ...
        try:
            ...
            if audio.frame_rate != 16000:
                wav = torchaudio.functional.resample(
                    wav, audio.frame_rate, 16000
                )

            # Apply denoising
            with torch.no_grad():
                wav = convert_audio(
                    wav, 16000, self.model.sample_rate, self.model.chin
                )
                denoised = self.model(wav.unsqueeze(0)).squeeze(0)

            # Convert back
            denoised_samples = denoised.cpu().numpy().flatten()
            denoised_samples = np.clip(
                denoised_samples * 32768, -32768, 32767
            ).astype(np.int16)
...
class AudioQualityAssessor:
    ...
    def assess_quality(audio: AudioSegment) -> AudioMetrics:
        ...
        try:
            ...
            # Fallback assessment
            return AudioMetrics(
                15.0, 9.0, 0.1, 0.05, AudioQuality.FAIR, 0.0, 16000, 1
            )
...
class UnifiedAudioPreprocessor:
    ...
    def _setup_capabilities(self):
        ...
        self.enable_noise_reduction = (
            self.config.enable_noise_reduction and NOISE_REDUCE_AVAILABLE
        )
        self.enable_vad = (
            self.config.enable_voice_activity_detection and VAD_AVAILABLE
        )
        self.enable_speech_enhancement = (
            self.config.enable_speech_enhancement
            and ADVANCED_PROCESSING_AVAILABLE
        )
        self.enable_facebook_denoiser = (
            self.config.enable_facebook_denoiser
            and FACEBOOK_DENOISER_AVAILABLE
        )
        self.enable_persian_optimization = (
            self.config.enable_persian_optimization
            and ADVANCED_PROCESSING_AVAILABLE
        )
    ...
    def _log_capabilities(self):
        ...
        self.logger.info(
            f"Unified audio preprocessing: {', '.join(caps) if caps else 'Basic mode'}"
        )
    ...
    def preprocess_audio(
        ...
        if self.adaptive_processing:
            ...
            self.logger.info(
                f"Audio quality: {metrics.quality_level.value} (SNR: {metrics.snr_db:.1f}dB)"
            )
        ...
        if self.enable_facebook_denoiser and (
            not metrics
            or metrics.quality_level in [AudioQuality.FAIR, AudioQuality.POOR]
        ):
        ...
        self.logger.info(
            f"Unified preprocessing completed. Final duration: {len(audio_data) / self.sample_rate:.2f}s"
        )
    ...
    def _reduce_noise(self, audio: AudioSegment) -> AudioSegment:
        ...
        try:
            ...
            reduced_samples = np.clip(
                reduced_samples * 32768, -32768, 32767
            ).astype(np.int16)
    ...
    def _apply_persian_optimization(self, audio: AudioSegment) -> AudioSegment:
        ...
        try:
            ...
            b, a = self.persian_optimizer.create_persian_emphasis_filter(
                self.sample_rate
            )
            ...
            consonant_enhanced = (
                self.persian_optimizer.enhance_persian_consonants(
                    emphasized_samples, self.sample_rate
                )
            )
            ...
            final_samples = (
                0.6 * samples
                + 0.3 * emphasized_samples
                + 0.1 * consonant_enhanced
            )
            final_samples = np.clip(
                final_samples * 32768, -32768, 32767
            ).astype(np.int16)
    ...
    def _enhance_speech(self, audio: AudioSegment) -> AudioSegment:
        ...
        try:
            ...
            # Convert back to int16
            enhanced_samples = np.clip(
                enhanced_samples * 32768, -32768, 32767
            ).astype(np.int16)
    ...
    def _final_normalization(self, audio: AudioSegment) -> AudioSegment:
        ...
        try:
            ...
            try:
                current_rms_db = (
                    audio.rms_db if hasattr(audio, "rms_db") else audio.dBFS
                )
            except:
    ...
    def detect_voice_activity(
        self, audio: AudioSegment, frame_duration_ms: int = 30
    ) -> List[Tuple[int, int]]:
        ...
        try:
            ...
            # Process in frames
            frame_size = (
                int(16000 * frame_duration_ms / 1000) * 2
            )  # 2 bytes per sample
            speech_segments = []
            ...
            final_segments = [
                (s, e) for s, e in merged_segments if e - s >= 1000
            ]
    ...
    def create_smart_chunks(
        ...
        try:
            ...
            for segment_start, segment_end in speech_segments:
                ...
                if segment_duration <= target_chunk_duration_ms:
                    ...
                    while current_pos < segment_end:
                        chunk_end = min(
                            current_pos + target_chunk_duration_ms, segment_end
                        )
            ...
            return self._fallback_chunking(
                audio, target_chunk_duration_ms, overlap_ms
            )

    def _fallback_chunking(
        self, audio: AudioSegment, chunk_duration_ms: int, overlap_ms: int
    ) -> List[Tuple[int, int]]:
    ...
    def save_processed_audio(
        self, audio_data: np.ndarray, output_path: str
    ) -> None:
        ...
        if SOUNDFILE_AVAILABLE:
            sf.write(
                output_path, audio_data, self.sample_rate, subtype="PCM_16"
            )
        else:
            # Fallback to scipy
