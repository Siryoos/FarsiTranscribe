...
class RepetitionDetector:
    ...
    @staticmethod
    def find_overlapping_sequences(text1: str, text2: str, min_overlap: int = 5) -> tuple:
        ...
        for i in range(len(words1) - min_overlap + 1):
            for j in range(len(words2) - min_overlap + 1):
                overlap_length = 0
                while (i + overlap_length < len(words1) and 
                       j + overlap_length < len(words2) and 
                       words1[i + overlap_length] == words2[j + overlap_length]):
                    overlap_length += 1
                
                if overlap_length > max_overlap and overlap_length >= min_overlap:
