...
class UnifiedTerminalDisplay:
    ...
    def _determine_optimal_display_mode(self) -> DisplayMode:
        ...
        if self.capabilities.rich_available and self.capabilities.rtl_libs_available and self.capabilities.rtl_support:
            return DisplayMode.RTL_RICH
        elif self.capabilities.rtl_libs_available and self.capabilities.rtl_support:
    ...
    def _log_capabilities(self):
        ...
        self.logger.info(f"Terminal capabilities: Unicode={self.capabilities.unicode_support}, "
    ...
    def print_header(self, title: str):
        ...
        if self.display_mode in [DisplayMode.RTL_RICH, DisplayMode.LTR_RICH] and self.console:
            # Rich formatting
    ...
    def format_transcription_table(self, transcriptions: List[str]) -> Optional[Table]:
        ...
        for i, transcription in enumerate(transcriptions, 1):
            ...
            if self.display_mode in [DisplayMode.RTL_RICH, DisplayMode.RTL_PLAIN]:
                display_text = self.rtl_processor.process_persian_text(display_text)
