...
class QualityAssessor:
    ...
    def assess_transcription_quality(self, transcription: str, audio_metadata: Dict[str, Any]) -> QualityMetrics:
        ...
        try:
            ...
            punctuation_accuracy = self._calculate_punctuation_accuracy(transcription)
            speaker_separation = self._calculate_speaker_separation(transcription)
            ...
            context_coherence = self._calculate_context_coherence(transcription)
            ...
            self.logger.info(f"Quality assessment complete. Overall score: {overall_score:.3f}")
            return metrics
    ...
    def _calculate_word_accuracy(self, transcription: str) -> float:
        ...
        for word in words:
            ...
            if persian_pattern.search(word):
                ...
                if len(word) >= 2 and not re.match(r'^[^\u0600-\u06FF]*$', word):
                    valid_words += 1
                else:
    ...
    def _calculate_punctuation_accuracy(self, transcription: str) -> float:
        ...
        if actual_ratio == 0:
            ...
            return max(0.0, 1.0 - abs(actual_ratio - expected_ratio) / expected_ratio)
    ...
    def _calculate_speaker_separation(self, transcription: str) -> float:
        ...
        conversation_indicators = [
            r'گفت', r'گفتند', r'پرسید', r'پاسخ داد', r'اضافه کرد',
            r'گفت:', r'گفتند:', r'پرسید:', r'پاسخ داد:', r'اضافه کرد:'
        ]
        
        indicator_count = sum(len(re.findall(pattern, transcription)) for pattern in conversation_indicators)
    ...
    def _calculate_confidence_score(self, audio_metadata: Dict[str, Any]) -> float:
        """Calculate confidence score from audio metadata."""
        # Extract confidence from metadata
        ensemble_confidence = audio_metadata.get('ensemble_confidence', 0.5)
        individual_confidences = audio_metadata.get('individual_confidences', {})
        ...
        if individual_confidences:
            ...
            if total_weight > 0:
                weighted_confidence = sum(
                    individual_confidences.get(name, 0.5) * weights.get(name, 0.1)
    ...
    def _calculate_repetition_score(self, transcription: str) -> float:
        ...
        repetition_ratio = sum(count - 1 for count in word_counts.values()) / total_words
        
        # Normalize to 0-1 scale
        return min(1.0, repetition_ratio * 2)  # Scale factor for better sensitivity
    ...
    def _calculate_context_coherence(self, transcription: str) -> float:
        ...
        connectors = [
            r'و', r'اما', r'ولی', r'اگر', r'چون', r'زیرا', r'بنابراین',
            r'همچنین', r'علاوه بر', r'در نتیجه', r'به عبارت دیگر'
        ]
        
        connector_count = sum(len(re.findall(connector, transcription)) for connector in connectors)
    ...
    def auto_tune_parameters(self, current_metrics: QualityMetrics, 
                           current_config: TranscriptionConfig) -> TranscriptionConfig:
        ...
        if current_metrics.word_accuracy < 0.8:
            ...
            optimized_config.chunk_duration_ms = min(30000, current_config.chunk_duration_ms + 5000)
        
        if current_metrics.sentence_fluency < 0.7:
            # Improve sentence fluency
            optimized_config.overlap_ms = min(1000, current_config.overlap_ms + 200)
        ...
        if current_metrics.noise_level > 0.5:
            # Reduce noise impact
            optimized_config.noise_threshold = max(0.2, current_config.noise_threshold - 0.1)
            optimized_config.enable_enhanced_preprocessing = True
        
        if current_metrics.repetition_score > 0.3:
            # Reduce repetition
            optimized_config.repetition_threshold = min(0.9, current_config.repetition_threshold + 0.05)
            optimized_config.max_word_repetition = max(1, current_config.max_word_repetition - 1)
        
        if current_metrics.confidence_score < 0.7:
            # Improve confidence
            optimized_config.min_chunk_confidence = max(0.5, current_config.min_chunk_confidence - 0.1)
            optimized_config.temperature = 0.0
...
"""
        ...
        if metrics.word_accuracy < 0.8:
            recommendations.append("• Improve word accuracy: Consider using larger models or better audio preprocessing")
        
        if metrics.sentence_fluency < 0.7:
            recommendations.append("• Improve sentence fluency: Increase overlap between chunks and enhance text post-processing")
        
        if metrics.punctuation_accuracy < 0.6:
            recommendations.append("• Improve punctuation: Enable text post-processing and use deterministic generation")
        
        if metrics.noise_level > 0.5:
            recommendations.append("• Reduce noise: Enhance audio preprocessing and adjust noise thresholds")
        
        if metrics.repetition_score > 0.3:
            recommendations.append("• Reduce repetition: Adjust repetition thresholds and improve deduplication")
