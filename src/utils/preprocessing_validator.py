...
class PreprocessingValidator:
    ...
    def validate_all_capabilities(self, config) -> ValidationResult:
        ...
        # Analyze results
        success = self._analyze_validation_results(warnings, errors, recommendations)
    ...
    def _validate_core_dependencies(self):
        ...
        for module_name, display_name in core_deps:
            capability = self._check_module_availability(module_name, display_name)
    ...
    def _check_module_availability(self, module_name: str, display_name: str) -> PreprocessingCapability:
    ...
    def _check_facebook_denoiser(self) -> PreprocessingCapability:
        ...
        try:
            ...
            denoiser_available = hasattr(torchaudio, 'functional') and hasattr(torchaudio.functional, 'spectral_centroid')
    ...
    def _analyze_validation_results(self, warnings: List[str], errors: List[str], recommendations: List[str]) -> bool:
        ...
        for name, capability in self.capabilities.items():
            if capability.required and not capability.available:
                errors.append(f"Required capability '{capability.name}' is not available: {capability.error_message}")
                success = False
            elif not capability.available and capability.error_message:
                warnings.append(f"Optional capability '{capability.name}' is not available: {capability.error_message}")
        
        # Generate recommendations
        if not self.capabilities.get("facebook_denoiser", PreprocessingCapability("", False)).available:
            recommendations.append("Consider installing torchaudio for advanced denoising capabilities")
        
        if not self.capabilities.get("adaptive_processing", PreprocessingCapability("", False)).available:
            recommendations.append("Consider installing scipy for adaptive processing features")
        ...
        try:
            ...
            if memory.available < 2 * 1024 * 1024 * 1024:  # Less than 2GB
                warnings.append("Available memory is low. Consider using memory-optimized configuration.")
        except:
            pass
    ...
    def get_capabilities_summary(self) -> Dict[str, bool]:
        ...
        return {name: capability.available for name, capability in self.capabilities.items()}
