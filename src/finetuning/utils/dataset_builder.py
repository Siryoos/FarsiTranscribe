...
class WhisperDatasetBuilder:
        ...
        try:
            ...
            if self.config.save_metadata:
                self._create_metadata(
                    data_pairs, train_data, val_data, test_data
                )
    ...
    def _collect_data_pairs(self) -> List[Dict[str, str]]:
        ...
        audio_transcript_map = self._create_audio_transcript_mapping(
        ...
        for audio_file, transcript_file in audio_transcript_map.items():
            try:
                ...
                transcript_text = self._validate_transcript_file(
                    transcript_file
                )
    ...
    def _create_audio_transcript_mapping(
        self, audio_files: List[Path], transcript_files: List[Path]
    ) -> Dict[Path, Path]:
        ...
        for audio_file in audio_files:
            ...
            transcript_file = self._find_corresponding_transcript(
                audio_file, transcript_files
            )
            if transcript_file:
    ...
    def _find_corresponding_transcript(
        self, audio_file: Path, transcript_files: List[Path]
    ) -> Optional[Path]:
        ...
        for transcript_file in transcript_files:
            if (
                audio_stem in transcript_file.stem
                or transcript_file.stem in audio_stem
            ):
                return transcript_file
    ...
    def _validate_audio_file(self, audio_file: Path) -> Optional[Dict]:
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to validate audio file {audio_file}: {str(e)}"
            )
            return None
    ...
    def _validate_transcript_file(
        ...
        try:
            ...
            if transcript_file.suffix.lower() == ".json":
                with open(transcript_file, "r", encoding="utf-8") as f:
                    ...
                    if isinstance(data, dict):
                        text = data.get(
                            "text",
                            data.get("transcript", data.get("caption", "")),
                        )
                    else:
                        text = str(data)
            else:
            ...
            self.logger.warning(
                f"Failed to read transcript {transcript_file}: {str(e)}"
            )
            return None
    ...
    def _create_dataset(self, data: List[Dict], split_name: str) -> Dataset:
        ...
        dataset = dataset.cast_column(
            "audio", Audio(sampling_rate=self.config.target_sample_rate)
        )
    ...
    def get_dataset_info(self) -> Dict:
        ...
        try:
            ...
            info = {
                ...
                "total_samples": sum(
                    len(split) for split in dataset_dict.values()
                ),
                "split_samples": {
                    split: len(dataset)
