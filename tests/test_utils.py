...
class TestRepetitionDetector:
    ...
    def test_detect_word_repetition(self):
        ...
        result = detector.detect_word_repetition(text, max_repetitions=2)
        # When max_repetitions=2 and there are 5 consecutive "hello", it keeps only 1
        assert result == "hello"
    ...
    def test_find_overlapping_sequences(self):
        ...
        overlap, start1, start2 = detector.find_overlapping_sequences(
            text1, text2, min_overlap=3
        )
        assert overlap >= 3
...
class TestSentenceExtractor:
    ...
    def test_split_into_paragraphs(self):
        ...
        paragraphs = extractor.split_into_paragraphs(
            text, min_paragraph_length=10
        )
        assert len(paragraphs) >= 2

        # Test short text
        paragraphs = extractor.split_into_paragraphs(
            "Short", min_paragraph_length=10
        )
        assert paragraphs == []
...
class TestTranscriptionFileManager:
    ...
    def setup_method(self):
        ...
        self.file_manager = TranscriptionFileManager(
            "test_file", self.temp_dir, self.config
        )
    ...
    def test_load_transcription(self):
        ...
        loaded_content = self.file_manager.load_transcription(
            use_cleaned=False
        )
        assert loaded_content == content
    ...
    def test_export_to_formats(self):
        ...
        exported_files = self.file_manager.export_to_formats(
            content, ["txt", "md"]
        )
