...
try:
    from src.finetuning.configs.finetuning_config import FineTuningConfig, DataConfig
except ImportError:
...
class DatasetDownloader:
    ...
    def download_file(self, url: str, filepath: Path, expected_size_mb: Optional[float] = None) -> bool:
        ...
        try:
            ...
            if expected_size_mb and actual_size < expected_size_mb * 1024 * 1024 * 0.9:
                self.logger.warning(f"File size seems too small: {actual_size / (1024*1024):.1f} MB")
            
            self.logger.info(f"Download completed: {actual_size / (1024*1024):.1f} MB")
            return True
    ...
    def extract_archive(self, archive_path: Path, extract_dir: Path, format_type: str) -> bool:
        """Extract downloaded archive."""
        try:
            self.logger.info(f"Extracting {archive_path.name} to {extract_dir}")
    ...
    def download_dataset(self, dataset_key: str, force_download: bool = False) -> bool:
        ...
        if dataset_dir.exists() and not force_download:
            self.logger.info(f"Dataset {dataset_key} already exists at {dataset_dir}")
            return True
        ...
        if not self.download_file(dataset['url'], archive_path, dataset['size_mb']):
            return False
        
        # Extract archive
        if not self.extract_archive(archive_path, self.output_dir, dataset['format']):
            return False
        ...
        if not dataset_dir.exists():
            self.logger.error(f"Expected dataset directory not found: {dataset_dir}")
            return False
        
        self.logger.info(f"Dataset {dataset_key} downloaded and extracted successfully")
        return True
    ...
    def _create_local_sample_dataset(self, dataset_dir: Path) -> bool:
        ...
        try:
            ...
            if examples_audio.exists():
                ...
                self.logger.info(f"Local sample dataset created successfully with {metadata['total_files']} files")
                return True
            else:
    ...
    def _create_sample_audio_dataset(self, dataset_dir: Path) -> bool:
        ...
        try:
            ...
            if examples_audio.exists():
                # Copy existing audio files
                import shutil
                audio_files = list(examples_audio.glob("*.m4a")) + list(examples_audio.glob("*.mp3")) + list(examples_audio.glob("*.wav"))
                ...
                if audio_files:
                    ...
                    self.logger.info(f"Sample audio dataset created successfully with {len(audio_files)} files")
                    return True
                else:
                    self.logger.warning("No audio files found in examples/audio directory")
                    return False
            else:
                self.logger.warning("No examples/audio directory found")
                return False
    ...
    def _download_huggingface_dataset(self, dataset: Dict, dataset_dir: Path) -> bool:
        """Download a HuggingFace dataset."""
        try:
            self.logger.info(f"Downloading HuggingFace dataset: {dataset['name']}")
            ...
            dataset_info = {
                ...
                "features": str(ds['train'].features) if 'train' in ds else "No train split",
            ...
            if 'train' in ds and len(ds['train']) > 0:
                sample_data = ds['train'].select(range(min(10, len(ds['train']))))
                sample_data.save_to_disk(str(dataset_dir / "sample"))
                self.logger.info(f"Saved sample data with {len(sample_data)} examples")
            
            self.logger.info(f"HuggingFace dataset {dataset_name} downloaded successfully")
            return True
    ...
    def download_all_datasets(self, force_download: bool = False) -> Dict[str, bool]:
        ...
        for dataset_key in self.datasets:
            self.logger.info(f"\n{'='*50}")
            self.logger.info(f"Processing dataset: {self.datasets[dataset_key]['name']}")
...
def main():
    ...
    parser.add_argument(
        "--info",
        action="store_true",
        help="Create dataset information file"
    )
