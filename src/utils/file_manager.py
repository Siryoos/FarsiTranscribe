...
class TranscriptionFileManager:
    ...
    def __init__(self, base_filename: str, output_directory: str, config: TranscriptionConfig):
        ...
        self.metadata_file_path = self.output_directory / f"{base_filename}_metadata.json"
    ...
    def _save_metadata(self, original_content: str, cleaned_content: str) -> None:
        ...
        metadata = {
            ...
            "reduction_percentage": ((len(original_content) - len(cleaned_content)) / len(original_content) * 100) if original_content else 0,
        ...
        for file_type, file_path in [
            ...
            if info[f"{file_type}_exists"]:
                try:
                    ...
                    self.logger.error(f"Error reading {file_type} file info: {e}")
        
        return info
    
    def load_transcription(self, use_cleaned: bool = True) -> str:
        """Load transcription from file."""
        file_path = self.cleaned_file_path if use_cleaned else self.unified_file_path
    ...
    def export_to_formats(self, content: str, formats: list = None) -> Dict[str, str]:
        ...
        for format_type in formats:
            try:
                if format_type == "txt":
                    file_path = self.output_directory / f"{self.base_filename}_transcription.txt"
                    with open(file_path, "w", encoding="utf-8") as file:
                        file.write(content)
                    exported_files["txt"] = str(file_path)
                
                elif format_type == "md":
                    file_path = self.output_directory / f"{self.base_filename}_transcription.md"
                    with open(file_path, "w", encoding="utf-8") as file:
                        file.write(f"# Transcription: {self.base_filename}\n\n")
                        file.write(content)
                    exported_files["md"] = str(file_path)
                
                elif format_type == "json":
                    import json
                    file_path = self.output_directory / f"{self.base_filename}_transcription.json"
                    data = {
