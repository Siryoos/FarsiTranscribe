...
class SpeakerDiarizer:
    ...
    def __init__(self, config: TranscriptionConfig):
        ...
        self.min_speaker_duration = 2.0  # Minimum segment duration in seconds
    ...
    def diarize_audio(self, audio_data: np.ndarray, sample_rate: int) -> List[SpeakerSegment]:
        ...
        try:
            ...
            speaker_embeddings = self._extract_speaker_embeddings(audio_data, sample_rate, speech_segments)
            ...
            self.logger.info(f"Diarization complete. Found {len(set(s.speaker_id for s in diarized_segments))} speakers")
    ...
    def _extract_speaker_embeddings(self, audio_data: np.ndarray, sample_rate: int, 
                                  speech_segments: List[Tuple[float, float]]) -> List[np.ndarray]:
    ...
    def _create_speaker_segments(self, audio_data: np.ndarray, sample_rate: int,
                               speech_segments: List[Tuple[float, float]], 
                               speaker_clusters: List[int]) -> List[SpeakerSegment]:
    ...
    def merge_similar_speakers(self, segments: List[SpeakerSegment]) -> List[SpeakerSegment]:
        ...
        for speaker_id, speaker_segments in speaker_groups.items():
            ...
            for segment in speaker_segments:
                if current_segment is None:
                    ...
                    if time_gap < 1.0:  # Merge if gap is less than 1 second
                        # Merge audio data
                        gap_samples = int(time_gap * self.config.target_sample_rate)
                        ...
                        current_segment = SpeakerSegment(
                            ...
                            confidence=min(current_segment.confidence, segment.confidence)
                        )
                    else:
